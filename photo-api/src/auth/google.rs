...
use oauth2::{
  ...
  reqwest::{http_client, Error as Oauth2ReqwestError},
  AuthUrl, AuthorizationCode, ClientId, ClientSecret, CsrfToken, EmptyExtraTokenFields,
  RedirectUrl, RequestTokenError, Scope, StandardErrorResponse, StandardTokenResponse,
...
pub fn get_user_profile(token: &BasicToken) -> Result<GoogleProfile> {
  ...
  let response = client
    ...
    .headers(headers)
    .send()
    .context(ReqwestIssue)?;

  let profile: GoogleProfile = response.json().context(JsonParse)?;
